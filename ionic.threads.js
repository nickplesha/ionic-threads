	...
	.directive('ionComment', function() {
		return {
			...
			template: '<ion-item ng-if="comment.data.body.length" class="ion-comment item">\
						<div class="ion-comment--author">{{comment.data.author}}<span class="ion-comment--op" ng-if="isOp()"> [OP]</span>:</div>\
			...
			controller: function($scope) {
				...
				$scope.isOp = function() {
					if ($scope.comment.data.author === $scope.op) {
						return true;
					}
					return false;
				}
